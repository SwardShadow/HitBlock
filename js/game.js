(function(){
    //游戏类
    //Game类的实例，是全局变量。所以，其他类的实例也能够方便的使用Game类的属性和方法
    window.Game=Class.extend({
        ...
        init:function(fps){
            ...
            //命令资源管理器加载图片资源，放入imageResource对象中
            var self=this;
            this.resourceManager.loadImage("data/res.json",function(alreadyLoadNumber,allNumber,imagesResourceObj){
                //清屏
                ...
                //如果已经加载好的图片数量等于所有图片的数量
                if(alreadyLoadNumber==allNumber){
                    //将返回来的资源对象，保存为自己的属性
                    self.imagesResources=imagesResourceObj;
                    //命令游戏开始
                    self.run(fps);
                }
            });
        ...
        run:function(fps){
            ...
            /*下面的一些语句是游戏开始的时候，实例化的一些东西。
            为什么不在init中实例化这些东西，必须在run中生成呢？
            这是因为init中的语句必须都执行完毕了，才能真的实例化一个game对象
            而ball，racket，blockManager类都在使用game对象(它是一个全局对象)
            所以，如果在init中实例化这些对象，那么ball，racker，blockManbager将无法使用game对象
            */ 
            //实例化一个小球
            this.ball=new Ball();
            //实例haul一个挡板
        ...
        mainloop:function(){
            ...
            //更新，渲染所有的砖块。这里使用的是砖块管理器提供的方法
            this.blockManager.updateAllBlocks();
            this.blockManager.updateAllBlocks(this.crx,this.imagesResources.block);
            //渲染球拍
            this.racket.render(this.ctx,this.imagesResources.racket);
            //更新渲染小球
            this.ball.update();
            this.ball.render(this.ctx,this.imagesResources.ball);
        },
        //暂停
        pause:function(){
            this.paused=true;
